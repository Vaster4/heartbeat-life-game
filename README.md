...
- 若与任意相邻盘子存在**同类型酒杯**，则该盘子与所有相邻盘子组成**参与组**
- 参与组通过 BFS 扩展：只要相邻盘子间有共同酒杯类型，就纳入同一组
- 参与组内所有酒杯按类型归类，**每种类型集中到一个盘子**上
  - 盘子按**放置时间从早到晚**排序
  - 类型按**数量从多到少**排序
  - 最早的盘子承载最多的类型，依次分配
  - 每个盘子最多容纳 6 个酒杯
- 多余的盘子（类型数 < 盘子数）变空后自动消除
- 消除后若产生新的相邻共同类型，继续触发**连锁反应**，直到棋盘达到**稳定态**
